const fbpiDebug=false;const fpbi="0.2.4";Hooks.on('renderActorSheet',function(obj,html){const actor=obj.actor;if(!(actor.data.type==="character")){return;}
if(actor.canUserModify(game.user,"update")==false){return;}
let element=html.find(".window-header .window-title");if(element.length!=1)return;let button=$(`<a class="popout" style><i class="fas fa-book"></i>Import from Pathbuilder 2e</a>`);button.on('click',()=>beginPathbuilderImport(obj.object));element.after(button);});var applyChanges=false;var finishedFeats=false;var finishedActions=false;var finishedClassFeatures=false;var finishedAncestryFeatures=false;var finishedEquipment=false;var finishedSpells=false;var addFeats=false;var addEquipment=false;var addMoney=false;var addSpellcasters=false;var deleteAll=false;var allItems=[];var jsonBuild=[];var addedItems=[];function beginPathbuilderImport(targetActor){applyChanges=false;finishedFeats=false;finishedActions=false;finishedClassFeatures=false;finishedAncestryFeatures=false;finishedEquipment=false;finishedSpells=false;allItems=[];new Dialog({title:`Pathbuilder Import`,content:`
...
addFeats=html.find('[name="checkBoxFeats"]')[0].checked;addEquipment=html.find('[name="checkBoxEquipment"]')[0].checked;addMoney=html.find('[name="checkBoxMoney"]')[0].checked;addSpellcasters=html.find('[name="checkBoxSpells"]')[0].checked;deleteAll=html.find('[name="checkBoxDeleteAll"]')[0].checked;fetchPathbuilderBuild(targetActor,buildID);}}}).render(true);}
...
function checkCharacterIsCorrect(targetActor,jsonBuild){let correctCharacter=false;new Dialog({title:jsonBuild.name,content:`
      ...
      `,buttons:{yes:{icon:"<i class='fas fa-check'></i>",label:`Proceed`,callback:()=>correctCharacter=true},no:{icon:"<i class='fas fa-times'></i>",label:`Cancel`},},default:"yes",close:html=>{if(correctCharacter){importCharacter(targetActor,jsonBuild);}}}).render(true);}
...
if(addFeats){finishedAncestryFeatures=true;finishedClassFeatures=true;addFeatItems(targetActor,arrayFeats);addActionItems(targetActor,arraySpecials);}else{finishedFeats=true;finishedAncestryFeatures=true;finishedActions=true;finishedClassFeatures=true;checkAllFinishedAndCreate(targetActor);}
...
for(const action of content.filter(item=>equipmentIsRequired(item,arrayEquipment,arrayWeapons,arrayArmor,arrayKit,addMoney))){for(var ref in arrayEquipment){if(arrayEquipment.hasOwnProperty(ref)){var itemName=arrayEquipment[ref][0];if(isNameMatch(itemName,action.data.data.slug)&&needsNewInstanceofItem(targetActor,itemName)){var itemAmount=arrayEquipment[ref][1];arrayEquipment[ref].added=true;const clonedData=JSON.parse(JSON.stringify(action.data));if(clonedData.type!="kit"){clonedData.data.quantity.value=itemAmount;allItems.push(clonedData);}}}}
...
function isNameMatch(pathbuilderItemName,foundryItemSlug){if(getSlug(pathbuilderItemName)==foundryItemSlug)return true;if(getSlug(getClassAdjustedSpecialNameLowerCase(pathbuilderItemName))==foundryItemSlug)return true;return false;}
...
function featIsRequired(item,arrayFeats){for(var featDetails in arrayFeats){if(arrayFeats.hasOwnProperty(featDetails)){if(getSlug(arrayFeats[featDetails][0])==item.data.data.slug){return true;}
if(getSlug(getClassAdjustedSpecialNameLowerCase(arrayFeats[featDetails][0]))==item.data.data.slug)return true;}}
return false;}
function specialIsRequired(item,arraySpecials){for(var ref in arraySpecials){if(arraySpecials.hasOwnProperty(ref)){if(getSlug(arraySpecials[ref])==item.data.data.slug)return true;if(getSlug(getClassAdjustedSpecialNameLowerCase(arraySpecials[ref]))==item.data.data.slug)return true;}}
return false;}
function equipmentIsRequired(item,arrayEquipment,arrayWeapons,arrayArmor,arrayKit,addMoney){for(var ref in arrayEquipment){if(arrayEquipment.hasOwnProperty(ref)){if(getSlug(arrayEquipment[ref][0])===item.data.data.slug)return true;}}
for(var ref in arrayWeapons){if(arrayWeapons.hasOwnProperty(ref)){if(getSlug(arrayWeapons[ref].name)===item.data.data.slug)return true;}}
for(var ref in arrayArmor){if(arrayArmor.hasOwnProperty(ref)){if(getSlug(arrayArmor[ref].name)===item.data.data.slug)return true;}}
...
return false;}
async function addSpecificCasterAndSpells(targetActor,spellCaster,magicTradition,spellcastingType){const spellcastingEntity={ability:{type:'String',label:'Spellcasting Ability',value:spellCaster.ability,},focus:{pool:spellCaster.focusPoints,},proficiency:{value:spellCaster.proficiency/2,},spelldc:{type:'String',label:'Class DC',item:0,},tradition:{type:'String',label:'Magic Tradition',value:magicTradition,},prepared:{type:'String',label:'Spellcasting Type',value:spellcastingType,},slots:{slot0:{max:spellCaster.perDay[0],prepared:[],value:0},slot1:{max:spellCaster.perDay[1],prepared:[],value:0},slot2:{max:spellCaster.perDay[2],prepared:[],value:0},slot3:{max:spellCaster.perDay[3],prepared:[],value:0},slot4:{max:spellCaster.perDay[4],prepared:[],value:0},slot5:{max:spellCaster.perDay[5],prepared:[],value:0},slot6:{max:spellCaster.perDay[6],prepared:[],value:0},slot7:{max:spellCaster.perDay[7],prepared:[],value:0},slot8:{max:spellCaster.perDay[8],prepared:[],value:0},slot9:{max:spellCaster.perDay[9],prepared:[],value:0},slot10:{max:spellCaster.perDay[10],prepared:[],value:0},},showUnpreparedSpells:{value:true}};var fake=[];const data={name:spellCaster.name,type:'spellcastingEntry',data:spellcastingEntity};fake.push(data);let spellCasterInstance=await targetActor.createEmbeddedDocuments('Item',fake);if(fbpiDebug)
...
warning+="</ul><br>";if(notAddedCount>0){new Dialog({title:`Pathbuilder Import Warning`,content:warning,buttons:{yes:{icon:"<i class='fas fa-check'></i>",label:`Finished`},},default:"yes",close:html=>{}}).render(true);}}}}
function getSlug(itemName){return itemName.toLowerCase().replace(/[^a-z0-9]+/gi,' ').trim().replace(/\s+|-{2,}/g,'-');}
